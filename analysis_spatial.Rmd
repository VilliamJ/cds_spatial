...
for (group_name in names(age_groups)) {
  ...
  # Total relocations per municipality for current age group
  relocations_per_muni <- df %>%
    group_by(dest) %>%
    summarise(total_relocations = sum(across(as.character(age_range)), na.rm = TRUE)) %>%
    ungroup()

  # Merge with population and compute popularity index (per 1000 residents)
  relocations_with_pop <- relocations_per_muni %>%
  ...
  # Join with spatial data
  muni_group_sf <- munic_sf %>%
    left_join(relocations_with_pop, by = c("NAME_2" = "dest")) %>%
    filter(!is.na(popularity_index))
...
# ─── Safety Check ──────────────────────────────────────────────────────
if (all(is.na(popularity_all$popularity_index))) {
  stop("🚫 All popularity_index values are NA. Check name mismatches between df$dest and population names.")
}

# Set color scale range — fixed min to avoid log(0)
global_min <- 0.1
global_max <- max(popularity_all$popularity_index, na.rm = TRUE)

if (!is.finite(global_max)) {
  stop("🚫 Global max popularity index is not finite.")
}
...
if (all(file.exists(image_files))) {
  ...
  warning("⚠️ One or more image files are missing.")
}
